...
export const convertScheduledTaskToViewScheduledTask = (
  scheduledTask: ScheduledTaskDto
) => {
  if (scheduledTask === defaultScheduledTaskDto) return undefined;

  const completedDate = isTaskCompleted(scheduledTask.completedDate)
    ? dayjs(scheduledTask.completedDate)
    : undefined;

  const completedIn =
    scheduledTask.completedInMinutes !== 0
      ? scheduledTask.completedInMinutes
      : '';
